...
def read_all():
    if os.path.exists("./merged_backtest_long_and_short.csv"):
        df = pd.read_csv("./merged_backtest_long_and_short.csv")
    ...
    merged_copy.to_csv("./merged_backtest_long_and_short.csv", index=False)
    return merged
...
MODEL = None
DATA = None
MODEL_PATH = from_relative_path("models-bybit/RL_LONG_AND_SHORT/best_model.zip")
...
if __name__ == "__main__":
    ...
    pd.DataFrame(results).to_csv(from_relative_path("prediction-bybit/RL_LONG_AND_SHORT.csv"), index=False)
