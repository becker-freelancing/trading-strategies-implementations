...
class CnnModelTraining(RegressionBaseTraining):
    ...
    def __init__(self):
        super().__init__("cnn_2_in_out",
                         DataSource.HIST_DATA,
                         Pair.EURUSD_5,
                         pd.Timestamp(year=2023, month=10, day=1),
                         pd.Timestamp(year=2023, month=12, day=31))
        self.scaler = MinMaxScaler()

    def build_model(self) -> Model:
        model = build_model((2, 1), 2)
    ...
    def prepare_data_scaled(self, data_source: DataSource, pair: Pair) -> tuple[np.ndarray, np.ndarray]:
        ...
        parts = self.slice_partitions(df, pair.minutes(), 4)
        print("Extracting Input data...")
        inputs = np.stack([part["closeBid"].to_numpy()[:2] for part in parts])
        print("Extracting Output data...")
        outputs = np.stack([part["closeBid"].to_numpy()[2:] for part in parts])
