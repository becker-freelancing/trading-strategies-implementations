...
for df in [dfs[2]]:
    ...
    up = (df["EMA_50"] > df["EMA_100"]) & (df["EMA_100"] > df["EMA_200"])
    df["Trend"].loc[up[up == True].index] = 1
    down = (df["EMA_50"] < df["EMA_100"]) & (df["EMA_100"] < df["EMA_200"])
    ...
    ups = len(up[up == True])
    downs = len(down[down == True])
    print("UP: ", ups / len(df) * 100)
    print("DOWNS: ", downs / len(df) * 100)
    print("SIDE: ", len(side[side == True]) / len(df) * 100)
