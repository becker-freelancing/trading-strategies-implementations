...
class RegressionModelTrainer(ModelTrainer):
    ...
    def _create_unsplited_data(self, regime: ModelMarketRegime, load_train_data=True) -> tuple[
        list[pd.DataFrame], list[pd.DataFrame], list[np.ndarray], int]:
        if load_train_data:
            ...
            return transformed_slices, slices, reduced_slices, self._get_target_column_idx(complete_data)
        else:
            slices, complete_data, self.regime_detector, self.model_regime_detector = self._load_model_data_for_regime(
                validation_data,
                                                                                           regime,
    ...
    def _get_train_data(self, regime: ModelMarketRegime) -> tuple[np.ndarray, np.ndarray]:
        train_data, original_data, reduced_train_data, target_column_idx = self._create_unsplited_data(regime,
                                                                                                       load_train_data=True)
        return self._parse_input_output(train_data, original_data, reduced_train_data, target_column_idx)

    def _get_validation_data(self, regime: ModelMarketRegime) -> tuple[np.ndarray, np.ndarray]:
        validation_data, original_data, reduced_train_data, target_column_idx = self._create_unsplited_data(regime,
                                                                                             load_train_data=False)
        return self._parse_input_output(validation_data, original_data, reduced_train_data, target_column_idx)

    def _parse_input_output(self, data: list[pd.DataFrame],
                            original_data: list[pd.DataFrame],
                            reduced_data: list[np.ndarray], target_column_idx) -> tuple[np.ndarray, np.ndarray]:
        input_data, output_data = self._create_input_output_sequences(data, original_data, reduced_data,
                                                                      target_column_idx)
