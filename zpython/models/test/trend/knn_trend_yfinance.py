...
# Get new features
df['RSI'] = ta.rsi(df["Close"]["EURUSD=X"])
df['ATR'] = ta.atr(df["High"]["EURUSD=X"], df["Low"]["EURUSD=X"], df["Close"]["EURUSD=X"], length=20)
df['Average'] = ta.midprice(df["High"]["EURUSD=X"], df["Low"]["EURUSD=X"], length=1)
df['MA40'] = ta.sma(df["Close"]["EURUSD=X"], length=40)
df['MA80'] = ta.sma(df["Close"]["EURUSD=X"], length=80)
df['MA160'] = ta.sma(df["Close"]["EURUSD=X"], length=160)

# Plot open price and moving averages
plt.figure(figsize=(14, 4))
plt.plot(df.index, df["Open"]["EURUSD=X"])
...
def get_target(barsupfront, df):
    length = len(df)
    high = list(df['High']["EURUSD=X"])
    low = list(df['Low']["EURUSD=X"])
    close = list(df['Close']["EURUSD=X"])
    open = list(df['Open']["EURUSD=X"])
