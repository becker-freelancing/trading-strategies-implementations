...
def execute_for_regressor(df, time_frame, train_length, pred_length, split_idxs, tests_per_model, regressor):
    slices = slice_data_multiple(df, train_length, pred_length, split_idxs)
    regressor_name = regressor[0]
    fixed_test_regression = functools.partial(test_regression, regressor_supplier=regressor[1])
    with ThreadPoolExecutor(max_workers=tests_per_model) as executor:
        ...
        errors_for_regressor = accumulate_errors(regression_errors)
...
def filter_regressors(train_lengths, pred_length, executed, time_frame, regressor_names, regressors):
    base_mask = (executed["TestLength"] == pred_length) & (executed["TimeFrame"] == time_frame.minutes())

    names = []
    regs = []
    lengths = set()
    for train_length in train_lengths:
        base_mask_2 = base_mask & (executed["InputLength"] == train_length)
        for i in range(len(regressor_names)):
            mask = base_mask_2 & (executed["Model"] == regressor_names[i])
            filtered = executed[mask]
            if len(filtered) == 0:
                if regressor_names[i] not in names:
                    names.append(regressor_names[i])
                    regs.append(regressors[i])
                lengths.add(train_length)

    return names, regs, list(lengths)


def execute_for_regressors(df, train_lengths, pred_length, split_idxs, tests_per_model, errors, executed, time_frame,
                           regressor_names, regressors):
    fixed_execute_for_regressor = functools.partial(execute_for_regressor, df=df, pred_length=pred_length,
                                                    split_idxs=split_idxs, tests_per_model=tests_per_model,
                                                    time_frame=time_frame)
    regressor_names, regressors, train_lengths = filter_regressors(train_lengths, pred_length, executed, time_frame,
                                                                   regressor_names, regressors)
    if len(regressor_names) == 0:
        return errors

    permutations = list(product(train_lengths, list(zip(regressor_names, regressors))))
    with ThreadPoolExecutor(max_workers=len(regressor_names)) as executor:
        errors_per_regressor = executor.map(
            lambda perm: fixed_execute_for_regressor(regressor=perm[1], train_length=perm[0]), permutations)
        errors_per_regressor = list(errors_per_regressor)
        for curr_error in errors_per_regressor:
...
if __name__ == "__main__":
    ...
    try:
        for time_frame in time_frames:
            ...
            errors = execute_for_regressors(df, train_lengths, pred_length, split_idxs, tests_per_model, errors,
