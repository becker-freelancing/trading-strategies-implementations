...
public class EMA2WithLongEmaFilterStrategy extends BaseStrategy {


    private final EMAIndicator shortSma;
    private final EMAIndicator longSma;
    private final EMAIndicator validationEma;

    public EMA2WithLongEmaFilterStrategy(StrategyParameter parameter, int shortMaPeriod, int longMaPeriod) {
        super(parameter);
        barSeries.setMaximumBarCount(longMaPeriod + 5);
        ClosePriceIndicator closePriceIndicator = new ClosePriceIndicator(barSeries);
        shortSma = new EMAIndicator(closePriceIndicator, shortMaPeriod);
        longSma = new EMAIndicator(closePriceIndicator, longMaPeriod);
        this.validationEma = new EMAIndicator(closePriceIndicator, 200);
    }

    private LastTwoMaResults lastTwoMaValuesForTime(EMAIndicator smaIndicator, int barCount) {
    ...
    public Optional<EntrySignalBuilder> internalShouldEnter(EntryExecutionParameter entryParameter) {
        ...
        double validationEmaValue = validationEma.getValue(barCount).doubleValue();
        boolean onlyBuy = entryParameter.currentPrice().getCloseMid().isGreaterThan(validationEmaValue);

        return toEntrySignal(lastShortMaValues, lastLongMaValues, entryParameter.currentPrice(), onlyBuy);
    }

    private Optional<EntrySignalBuilder> toEntrySignal(LastTwoMaResults lastShortMaValues, LastTwoMaResults lastLongMaValues, TimeSeriesEntry current, boolean onlyBuy) {
        ...
        if (onlyBuy && lastShort < lastLong && currentShort > currentLong) {
            //BUY
        ...
        } else if (!onlyBuy && lastShort > lastLong && currentShort < currentLong) {
            //SELL
            Pair pair = current.pair();
            return Optional.ofNullable(entrySignalBuilder()
                    .withOpenMarketRegime(currentMarketRegime())
